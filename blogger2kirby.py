...
def markdownify(html):
	p = subprocess.Popen(['pandoc', '--normalize', '--smart',
		'--no-wrap', '-f', 'html', '-t', 'markdown_strict+footnotes', '-'],
...
def write_output(posts, comment_footers):
	for slug, article in posts.items():
		...
		foldername = "out/" + ("%04d%02d%02d-%s" % (article.date.year,
			article.date.month,
			article.date.day,
			slug))
		...
		for image in soup.findAll("img"):
			...
			image.replace_with("(image: %s)" % filename)

		stat("Opening file: %s" % os.path.join(foldername, 'article.text.txt'))
		with codecs.open(
